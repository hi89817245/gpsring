...
2. **作弊點不一定在基隆或 A/B 舍**。不法擄鴿集團可能在山區架設攔鳥網，真正要找的是「異常起點」與「滯留/轉運起點」，應用醒目圖示標示，方便會長快速處理。
3. **只看平面地圖不足以分析軌跡**。需要高度/時間維度小視窗，與大地圖互動比對位置、速度、高度、時間。
4. **規則碼不能整條路都一樣**。GPS-R01/R02/R03 應依區段與證據類型精準標註，並提供 help 與可調閥值。
5. **需要篩選器**。例如日期時間、GPS 高度 N 米以下、速度門檻、HDOP、衛星數、規則碼、風險等級，以便快速找出不尋常軌跡。
...
| 參數 | 預設 | 說明 |
|---|---:|---|
| `min_corridor_match_km` | 20 km | 至少連續 20 公里貼近高速/高鐵才升為高危 |
...
#### UI 行為
- 篩選後地圖只高亮符合條件的 segments/markers。
- 左側列表顯示符合條件的事件。
- 點擊事件 zoom 到該段。
...
優先順序：
1. 連續 20km corridor run 判定。
2. poaching_mountain_net 模擬資料。
3. 擄鴿/轉運起點圖示。
4. 高度/速度 timeline 小窗。
5. 規則 Help/閥值設定。
6. 篩選器。
